...
import { trackSubscribeClick, trackCheckoutStart, trackPaymentSuccess, trackPaymentFailure } from '@/lib/gtag';
...
export default function SubscriptionPage() {
  ...
  const handleSubscribe = async () => {
    ...
    trackCheckoutStart(selectedPlan.name);
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response: any) {
            try {
                ...
                trackPaymentSuccess(selectedPlan.name, finalPrice);

                toast({ title: 'Payment Successful!', description: 'Your subscription is now active.'});
                setIsDialogOpen(false);
            } catch (verifyError: any) {
                trackPaymentFailure(selectedPlan.name, verifyError.message);
        ...
        theme: { color: "#0B2447" },
        modal: { ondismiss: () => {
            trackPaymentFailure(selectedPlan.name, 'Checkout closed by user');
      ...
      rzp.on('payment.failed', (response: any) => {
          trackPaymentFailure(selectedPlan.name, response.error.description);
